...
const remotionProjectGeneratorTool = createTool({
...
function generateRootFile(title) {
...
function generateCompositionFile(script) {
  const scenesCode = script.scenes.map((scene) => {
    const codeField = scene.code ? `, code: ${JSON.stringify(scene.code)}` : "";
    return `    {
      id: "${scene.id}",
      type: "${scene.type}",
...
export { remotionProjectGeneratorTool };
