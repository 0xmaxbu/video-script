import fs from "node:fs";
import path from "node:path";
import { execSync } from "node:child_process";
import { fileURLToPath } from "node:url";
import type { ScriptOutput } from "../types.js";

// __dirname equivalent for ESM
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

export interface GenerateProjectInput {
  script: ScriptOutput;
  outputDir: string;
  images?: Record<string, string>;
  showSubtitles?: boolean;
  /**
   * When true, skips `npm install --legacy-peer-deps` after scaffolding.
   * All other scaffolding (Root.tsx, index.ts, package.json, tsconfig.json,
   * public/, .gitignore, README.md) is performed exactly as in production.
   * This option exists solely to speed up fixture-based tests that assert
   * file structure without requiring a full install. It MUST NOT alter the
   * renderer entry, Composition, Root, or props data path in any way.
   */
  skipInstall?: boolean;
}

export interface GenerateProjectOutput {
  projectDir: string;
  entryPoint: string;
  compositionId: string;
}

export async function generateProject(
  input: GenerateProjectInput,
): Promise<GenerateProjectOutput> {
  const {
    script,
    outputDir,
    images = {},
    showSubtitles = false,
    skipInstall = false,
  } = input;

  // Resolve paths to shared packages
  // At runtime: __dirname = packages/renderer/dist/utils/
  // ../.. from dist/utils/ → dist/ → packages/renderer/
  const rendererDir = path.resolve(__dirname, "../.."); // packages/renderer
  const typesDir = path.resolve(rendererDir, "../types"); // packages/types (if exists)
  const hasTypes = fs.existsSync(typesDir);

  const compositionId = "Video"; // always landscape for now

  // 1. Scaffold directory structure
  fs.mkdirSync(path.join(outputDir, "src"), { recursive: true });
  fs.mkdirSync(path.join(outputDir, "out"), { recursive: true });
  fs.mkdirSync(path.join(outputDir, "public"), { recursive: true });

  // Copy images to public/ so Remotion's webpack dev server can serve them.
  // Convert absolute paths → filenames only (e.g. "/abs/path/foo.png" → "foo.png").
  const resolvedImages: Record<string, string> = {};
  for (const [key, absPath] of Object.entries(images)) {
    const filename = path.basename(absPath);
    fs.copyFileSync(absPath, path.join(outputDir, "public", filename));
    resolvedImages[key] = filename;
  }

  // 2. Write package.json with file: protocol
  const packageJson: Record<string, unknown> = {
    name: `video-${slugify(script.title)}`,
    version: "1.0.0",
    private: true,
    dependencies: {
      "@video-script/renderer": `file:${rendererDir}`,
      ...(hasTypes ? { "@video-script/types": `file:${typesDir}` } : {}),
      react: "^19.0.0",
      "react-dom": "^19.0.0",
      remotion: "4.0.436",
      "@remotion/bundler": "4.0.436",
      "@remotion/renderer": "4.0.436",
      "@remotion/transitions": "4.0.436",
    },
  };
  fs.writeFileSync(
    path.join(outputDir, "package.json"),
    JSON.stringify(packageJson, null, 2),
  );

  // 3. Write tsconfig.json
  const tsconfig = {
    compilerOptions: {
      target: "ES2022",
      module: "ES2022",
      moduleResolution: "bundler",
      jsx: "react-jsx",
      strict: true,
      esModuleInterop: true,
      skipLibCheck: true,
      outDir: "dist",
    },
    include: ["src"],
  };
  fs.writeFileSync(
    path.join(outputDir, "tsconfig.json"),
    JSON.stringify(tsconfig, null, 2),
  );

  // 4. Write src/index.ts (Remotion entry point)
  // Use extensionless import './Root' — webpack resolves .tsx, tsc resolves .tsx too
  // Do NOT use './Root.js' — webpack cannot remap .js → .tsx like tsc can
  const indexContent = `import { registerRoot } from 'remotion';
import { RemotionRoot } from './Root';

registerRoot(RemotionRoot);
`;
  fs.writeFileSync(path.join(outputDir, "src", "index.ts"), indexContent);

  // 5. Write src/Root.tsx (embeds script data as defaultProps)
  // Use resolvedImages (filename-only values) so staticFile() works at render time
  const rootContent = generateRootTsx({
    script,
    images: resolvedImages,
    showSubtitles,
    compositionId,
  });
  fs.writeFileSync(path.join(outputDir, "src", "Root.tsx"), rootContent);

  // 6. Write .gitignore
  fs.writeFileSync(
    path.join(outputDir, ".gitignore"),
    "node_modules/\ndist/\nout/\n",
  );

  // 7. Write README.md
  fs.writeFileSync(
    path.join(outputDir, "README.md"),
    `# ${script.title}\n\nGenerated by video-script.\n\nPreview: \`npx remotion studio\`\n`,
  );

  // 8. Run npm install to resolve file: symlinks (skipped when skipInstall=true)
  if (!skipInstall) {
    execSync("npm install --legacy-peer-deps", {
      cwd: outputDir,
      stdio: "inherit",
    });
  }

  return {
    projectDir: outputDir,
    entryPoint: path.join(outputDir, "src", "index.ts"),
    compositionId,
  };
}

function generateRootTsx(opts: {
  script: ScriptOutput;
  images: Record<string, string>;
  showSubtitles: boolean;
  compositionId: string;
}): string {
  const { script, images, showSubtitles, compositionId } = opts;
  const totalFrames = Math.ceil(script.totalDuration * 30);
  const propsJson = JSON.stringify({ script, images, showSubtitles }, null, 2);

  return `import { Composition } from 'remotion';
import { VideoComposition } from '@video-script/renderer/remotion';
import type { VideoCompositionProps } from '@video-script/renderer/remotion';

const defaultProps: VideoCompositionProps = ${propsJson};

export const RemotionRoot = () => (
  <Composition
    id="${compositionId}"
    component={VideoComposition}
    durationInFrames={${totalFrames}}
    fps={30}
    width={1920}
    height={1080}
    defaultProps={defaultProps}
  />
);
`;
}

function slugify(title: string): string {
  return title
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-|-$/g, "");
}
