...
export const codeHighlightTool = createTool({
  ...
  inputSchema: z.object({
    ...
    language: z
      .string()
      .min(1)
      .describe('Programming language (javascript, typescript, python, go, rust, etc.)'),
    highlightLines: z
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('unknown language')) {
          throw new Error(`UNSUPPORTED_LANGUAGE: Language "${language}" is not supported by Shiki`);
        }
